...
def _get_prefixes_max_len():
    ...
    prefixes = [len(compressor.prefix) for compressor in list(_COMPRESSORS.values())]
    ...
    return ((sys.byteorder == 'big' and
             (array.dtype.byteorder == '<' or
              (array.dtype.byteorder == '|' and array.dtype.fields and
               all(e[0].byteorder == '<'
                   for e in list(array.dtype.fields.values()))))) or
            (sys.byteorder == 'little' and
             (array.dtype.byteorder == '>' or
              (array.dtype.byteorder == '|' and array.dtype.fields and
               all(e[0].byteorder == '>'
                   for e in list(array.dtype.fields.values()))))))
...
def _detect_compressor(fileobj):
    ...
    if first_bytes.startswith(_ZFILE_PREFIX):
        ...
        for name, compressor in list(_COMPRESSORS.items()):
...
def _write_fileobject(filename, compress=("zlib", 3)):
    ...
    if compressmethod in list(_COMPRESSORS.keys()):
