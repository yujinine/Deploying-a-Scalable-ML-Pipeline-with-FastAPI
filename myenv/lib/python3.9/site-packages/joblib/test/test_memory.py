...
def test_instanciate_incomplete_store_backend():
    ...
    assert (backend_name, IncompleteStoreBackend) in list(_STORE_BACKENDS.items())
    with raises(TypeError) as excinfo:
...
def test_dummy_store_backend():
    ...
    assert (backend_name, DummyStoreBackend) in list(_STORE_BACKENDS.items())
...
def compare(left, right, ignored_attrs=None):
    ...
    for attr in list(left_vars.keys()):
